										...
										float *smoothPath, int *smoothPathSize, int maxSmoothPathSize)
{
	*smoothPathSize = 0;
	...
	while (npolys && nsmoothPath < maxSmoothPathSize)
	{
		...
		if (!getSteerTarget(m_navMeshQuery, iterPos, targetPos, SMOOTH_PATH_SLOP, polys, npolys, steerPos, steerPosFlag, steerPosRef))
		{
			break;
		}
		...
		else if (offMeshConnection && inRangeYZX(iterPos, steerPos, SMOOTH_PATH_SLOP, 1.0f))
		{
			...
			dtResult = mesh->getOffMeshConnectionPolyEndPoints(prevRef, polyRef, newStartPos, newEndPos);
